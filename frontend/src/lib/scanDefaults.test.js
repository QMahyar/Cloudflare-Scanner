...
describe('scanDefaults', () => {
	...
	it('aligns endpoint defaults with scanner.go / scan_handlers.go', () => {
		...
		expect(ENDPOINT_DEFAULTS.concurrency).toBe('0')
	...
	it('aligns clean defaults with cleanip.go', () => {
		...
		expect(CLEAN_DEFAULTS.phase1Probes).toBe('0')
		expect(CLEAN_DEFAULTS.phase2Probes).toBe('16')
		expect(CLEAN_DEFAULTS.phase2Count).toBe('30')
	...
	it('offers Auto for phase-1 and full-batch steps for phase-2', () => {
		...
		expect(PHASE2_PROBES_OPTIONS.map((o) => o.v)).toEqual(['8', '16', '32', '48', '64', '128'])
