...
type cleanScanRequest struct {
	...
	// validated clean IP is swapped into.
...
func handleCleanScanStart(xrayPath string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.ConfigURL != "" {
			var err error
			cfg, err = ParseProxyURL(req.ConfigURL)
			if err != nil {
...
func handleCleanScanResults(w http.ResponseWriter, r *http.Request) {
	...
	if showPhase2 {
		...
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]interface{}{
			// entries = current phase view (phase2 successes when showPhase2).
			"entries":         entries,
...
func handleCleanExport(w http.ResponseWriter, r *http.Request) {
	...
	if req.ConfigURL == "" || len(req.Endpoints) == 0 {
