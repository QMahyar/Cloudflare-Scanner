...
if ! command -v pkg >/dev/null 2>&1 || [ ! -d /data/data/com.termux ]; then
	...
	echo "Linux:   curl -fsSL https://raw.githubusercontent.com/${REPO}/master/scripts/install-linux.sh | sh"
	echo "macOS:   curl -fsSL https://raw.githubusercontent.com/${REPO}/master/scripts/install-macos.sh | sh"
...
aarch64 | arm64) ;;
*)
	echo "Unsupported Termux architecture: $ARCH"
	echo "Only Android/Termux ARM64 is currently released."
	exit 1
	;;
esac
